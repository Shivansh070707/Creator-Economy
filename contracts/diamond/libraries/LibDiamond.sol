...
library LibDiamond {
    ...
    bytes32 constant CREATOR_STORAGE_POSITION =
        keccak256("diamond.standard.creator.storage");
    ...
    struct CreatorEconomyStorage {
        ...
        address i_inani;
        uint256 SUPPLY_CAP; // max supply that can be minted for a token
        uint256 INITIAL_SUPPLY; // goes to the creator
    ...
    function creatorEconomyStorage()
        internal
        pure
        returns (CreatorEconomyStorage storage c)
    {
        bytes32 position = CREATOR_STORAGE_POSITION;
        assembly {
            c.slot := position
        }
    }
