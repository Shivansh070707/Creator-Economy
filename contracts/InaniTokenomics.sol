...
error InaniTokenomics__AlreadyWhitelistedPrivateSale();
error InaniTokenomics__NotOwner();
error InaniTokenomics__NotEnoughBalanceForPrivateSale();
error InaniTokenomics__NotWhitelisted();
error InaniTokenomics__SaleNotRunning();
error InaniTokenomics_InvalidInvestmentAmount();
error InaniTokenomics__PrivateSalePeriodExpired();
error InaniTokenomics__NotEnoughTokensToSell();

contract InaniTokenomics {
    ...
    Inani public immutable i_inaToken;
    ...
    modifier notAlreadyWhiteListedPrivateSale(address _investor) {
        if (investorsPrivateSale[_investor].isWhitelisted) {
            revert InaniTokenomics__AlreadyWhitelistedPrivateSale();
        }
        _;
    }
    ...
    modifier onlyWhitelisted(address caller) {
        if (!investorsPrivateSale[caller].isWhitelisted) {
            revert InaniTokenomics__NotWhitelisted();
        }
        _;
    }
    ...
    modifier saleRunning() {
        if (!privateSale.isRunning) {
            revert InaniTokenomics__SaleNotRunning();
        }
        _;
    }
    ...
    modifier satisfyMinMax(string memory _symbol, uint256 _amount) {
        ...
        if (amountUsd < MIN_INVESTMENT_USD || amountUsd > MAX_INVESTMENT_USD) {
            revert InaniTokenomics_InvalidInvestmentAmount();
        }
        _;
    }
    ...
    modifier periodLeft() {
        if (
            ...
            revert InaniTokenomics__PrivateSalePeriodExpired();
        }
        _;
    }
    ...
    function whitelistForPrivateSale(address investorPrivateSale)
        external
        onlyOwner(msg.sender)
        notAlreadyWhiteListedPrivateSale(investorPrivateSale)
        returns (bool success)
    {
        InvestorPrivateSale memory newInvestor = InvestorPrivateSale(
            true,
            0,
            0
        );
        ...
        returns (bool success)
    {
        ...
        if (tokensToPurchase > privateSale.tokensLeft) {
            revert InaniTokenomics__NotEnoughTokensToSell();
        }
        ERC20 inputToken;
        ...
        investorsPrivateSale[msg.sender].totalAmountInvested +=
            amountUsd *
            (10**18);
    ...
    function getEquivalentUsd(string memory _symbol, uint256 _amount)
        private
        view
        returns (uint256)
    {
    ...
    function sendTokens() private {
        ...
        i_inaToken.transfer(i_owner, privateSale.tokensLeft);
        ...
        for (uint256 i = 0; i < whitelistedForPrivateSale.length; i++) {
            uint256 tokensToTransfer = investorsPrivateSale[
                whitelistedForPrivateSale[i]
            ].tokensToSend;
            i_inaToken.transfer(whitelistedForPrivateSale[i], tokensToTransfer);
        }
    }

    function getFoundingTeamTokens()
        public
        view
        onlyOwner(msg.sender)
        returns (uint256)
    ...
    function isWhitelistedForPrivateSale(address investor)
        public
        view
        returns (bool)
    {
